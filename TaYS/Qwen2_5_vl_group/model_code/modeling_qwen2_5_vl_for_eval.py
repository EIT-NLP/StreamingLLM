...
from .causal_mask_prefill import (
    build_group_custom_causal_mask,
)  # ,build_group_custom_causal_mask_decoding
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        input_ids_for_group=None,
        video_grid_thw=None,
        is_decoding: bool = False,  # This is used to determine if we are in the decoding phase
        ...
        causal_mask = self._update_causal_mask(
            ...
            input_ids_for_group,
            video_grid_thw,
            is_decoding=input_ids_for_group.shape[1] == 1,
    ...
    def _update_causal_mask(
        ...
        input_ids_for_group=None,
        video_grid_thw=None,
        is_decoding=False,  # This is used to determine if we are in the decoding phase
        token_len_schedule: Optional[List[int]] = None,
    ):
        ...
        try:
            ...
            for b in range(B):
                ...
                token_per_frame = (H // 2) * (W // 2)
                video_frame_token_counts.append(token_per_frame)
            ...
            if is_decoding:
                ...
                # print("NOT_decoding, use group mask")
            ...
            print("Error in build_group_custom_causal_mask")
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        second_per_grid_ts: Optional[torch.Tensor] = None,
        token_len_schedule: List[int] | None = None,
        ...
        if position_ids is None and past_key_values.get_seq_length() != 0:
            batch_size, seq_length, _ = inputs_embeds.shape
            position_ids = torch.arange(seq_length, device=inputs_embeds.device)
            position_ids = position_ids.view(1, -1).expand(batch_size, -1)
            position_ids = position_ids.add(self.final_position_id + 1)
            self.final_position_id = position_ids[-1, -1].item()
        ...
        outputs = self.model(
            ...
            is_decoding=input_ids.shape[1] == 1,
            token_len_schedule=token_len_schedule,
        )
        ...
        if labels is None:
            ...
            skip_prob = probs[SKIP_ID].item()

            topk_prob, topk_idx = torch.topk(probs, k=3)
            formatted_probs = ", ".join(f"{p:.5f}" for p in topk_prob.tolist())
            # print(
            #     f"skip_prob: {skip_prob:.4f}, top3_ids: {topk_idx.tolist()}, top3_probs: [{formatted_probs}]"
            # )
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        if model_inputs["position_ids"] != None:
            self.final_position_id = position_ids[-1, -1, -1].item()
