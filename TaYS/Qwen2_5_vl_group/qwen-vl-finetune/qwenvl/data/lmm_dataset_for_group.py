...
def get_rope_index_25(
    ...
    if (
    ...
    ):  # get the position of the first answer: np.where(input_ids[0] == assistant_id)[0][0]
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            if st < len(input_tokens):  # np.where(input_ids[0] == assistant_id)[0][0]
                st_idx = (
                    llm_pos_ids_list[-1].max() + 1 if len(llm_pos_ids_list) > 0 else 0
                )
                ...
                llm_pos_ids_list.append(
                    torch.arange(len(generation_tokens)).view(1, -1).expand(3, -1)
                )
...
class LMMDataset(Dataset):
    def __init__(
        ...
        try:
            if data_args:
                ...
                self.data_args.image_processor.size["longest_edge"] = (
                    data_args.max_pixels
                )
                self.data_args.image_processor.size["shortest_edge"] = (
                    data_args.min_pixels
                )
        except:
            if data_args:
                self.data_args.image_processor = getattr(
                    self.data_args, "image_processor", None
                )
    ...
    def getitem(self, index):
        ...
        if sample["answer"]:
            ...
            # <answer>        → 151668
            # </answer>       → 151669
            combined_text = human_caption + "<answer>" + sample["answer"] + "</answer>"
        else:
            combined_text = human_caption
        ...
        conversation = [
            ...
            },
            {
                ...
                "content": [
                    {
                        "type": "text_stream",
                        "text_stream": [[video_start, video_end, combined_text]],
                    }
                ],
            },
        ]
        ...
        THINK_END = 151667
        if sample["answer"]:
            if (input_ids == THINK_END).sum().item() != video_inputs[0].shape[
                0
            ] // 2 - 1:
                print("❌Bad sample", question)
                print(
                    (input_ids == THINK_END).sum().item(),
                    video_inputs[0].shape[0] // 2,
                )
...
if __name__ == "__main__":
    ...
    dataset = LMMDataset(
        annotation_paths=[
            # "/code/data/PE-Video/train_PE_token_2plus.jsonl",
            "/code/data/VideoEspresso/videoespresso_train_video_duration_le_40s_rewrited_completed.jsonl",
            # 'llava_video_178k_with_seeks.jsonl',
            # 'llava_hound_video_with_seeks.jsonl',
            # 'llava_ov_multi_image_with_seeks.jsonl',
            # 'llava_ov_single_image_text_mix_with_seeks.jsonl'
