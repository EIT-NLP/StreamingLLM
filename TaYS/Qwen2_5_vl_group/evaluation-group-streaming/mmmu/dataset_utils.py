...
def load_dataset_livesports(
    is3_5: bool = True,
    testsplit: str = "2plus",
    json_path: str = "/code/chr/download/PE-Video/test_3_5.jsonl",
    ...
    print("------------is3_5:", is3_5)
    if testsplit == "none":
        json_path = "/code/chr/download/PE-Video/test_2plus.jsonl"
    elif is3_5:
        json_path = f"/code/chr/download/PE-Video/test_3_5_scene_{testsplit}.jsonl"
    else:
        json_path = f"/code/chr/download/PE-Video/test_2plus_scene_{testsplit}.jsonl"
    with open(json_path, "r", encoding="utf-8") as f:
        for line in f:
            if not line.strip():
                continue
            try:
                ...
                #     continue
                # elif 'mkv' in video_path:
                #     print(f"⚠️ Warning: Video file {video_path} is in MKV format. Skipping.")
                #     continue
                # else:
                #     print(f"✅ Found video: {video_path}")
