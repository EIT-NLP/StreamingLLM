...
def run_inference(args):
    ...
    for i in tqdm(range(len(data)), desc="Running inference"):
        ...
        response, timing_info = model.generate(
            messages, line["video_start"], line["video_end"]
        )

        if args.CoT:
            print(f"response_with_think: {response}")
        else:
...
def main():
    ...
    infer_parser.add_argument(
        "--model-path",
        type=str,
        default="/code/Streaming_video/pe_version/Qwen2_5_vl_batch/qwen-vl-finetune/output/qwen2_5vl-pe-batch-waitkey-random-20k",
        help="Path to the model",
    )
    infer_parser.add_argument(
        "--dataset", type=str, default="LiveSports-3K", help="Dataset name"
    )
    infer_parser.add_argument(
        "--data-dir",
        type=str,
    ...
    infer_parser.add_argument(
        "--output-file",
        type=str,
        default="/code/Streaming_video/pe_version/Qwen2_5_vl_batch/evaluation/mmmu/output/PE-batch-randomkey-20k.json",
