...
def clean_think_answer_text(text: str):
    """
    策略：
      1) 删除所有非允许的标签（只保留 <think>, </think>, <answer>, </answer>）；
      2) 提取并删除最后一个完整的 <answer>...</answer>，作为 final_answer；
      3) 在剩余文本中，找到所有被任意两个标签包裹的非空文本段，并对这些文本段再去掉任何残留的 <...> 标签，作为 think；
      4) 按出现顺序拼回：<think>思考1</think><think>思考2</think>...<answer>最终答案</answer>
    """
    ...
    final_str = f"{think_str}<answer>{final_answer}</answer>"
    return final_str
