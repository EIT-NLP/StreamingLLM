    ...
    def get_rope_index(
        ...
        if (
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                if st < len(
                    input_tokens
                ):  # np.where(input_ids[0] == assistant_id)[0][0]
                    st_idx = (
                        llm_pos_ids_list[-1].max() + 1
                        if len(llm_pos_ids_list) > 0
                        else 0
                    ...
                    llm_pos_ids_list.append(
                        torch.arange(len(generation_tokens), device=device)
                        .view(1, -1)
                        .expand(3, -1)
                    )
    ...
    def forward(
        ...
        if position_ids is None and (
            ...
            if (
                ...
                self.input_length = position_ids.shape[-1]
                self.last_position_ids = position_ids[0, 0, -1]
            ...
            # then use the prev pre-calculated rope-deltas to get the correct position ids
                ...
                delta = (
                    (
                        self.rope_deltas - self.rope_deltas + self.last_position_ids + 1
                    ).to(inputs_embeds.device)
                    if cache_position is not None
                    else 0
                )

                self.last_position_ids = self.last_position_ids + 1
