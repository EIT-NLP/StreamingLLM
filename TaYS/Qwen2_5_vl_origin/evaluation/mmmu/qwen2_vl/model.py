...
sys.path.insert(0, "/code/Streaming_video/pe_version/Qwen2_5_vl_origin")
import logging
import math
import warnings
...
from .util import auto_split_flag, get_gpu_memory, get_rank_and_world_size, listinstr
...
class Qwen2VLChat(Qwen2VLPromptMixin, BaseModel):
    ...
    def __init__(
        ...
        if listinstr(["2.5", "2_5", "qwen25"], model_path.lower()):
            # sys.path.append("/code/Streaming_video/pe_version/Qwen2_5_vl_origin/model_code")
            ...
            try:
                self.processor = AutoProcessor.from_pretrained(
                    "Qwen/Qwen2.5-VL-7B-Instruct", padding_side="right"
                )
            except:
                self.processor = AutoProcessor.from_pretrained(
    ...
    def generate_inner(self, message, video_start, video_end, dataset=None):
        try:
            ...
            sys.path.append(
                "/code/Streaming_video/pe_version/Qwen2_5_vl_origin/qwen-vl-utils/src"
            )
