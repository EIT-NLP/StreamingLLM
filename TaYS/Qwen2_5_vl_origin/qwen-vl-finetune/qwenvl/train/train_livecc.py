...
def add_special_tokens_safely(tokenizer, new_tokens):
    ...
    if not tokens_to_add:
        print("🟢 所有指定的 token 已存在于词表中，无需添加。")
        return False
    ...
    print(f"📌 正在添加新 token: {tokens_to_add}")
    print(f"🔧 更新后的 additional_special_tokens 总数: {len(updated_special_tokens)}")
    ...
    if num_added > 0:
        print(f"✅ 成功添加 {num_added} 个新 token 到词表")
