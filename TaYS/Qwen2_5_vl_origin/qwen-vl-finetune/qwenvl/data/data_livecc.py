...
def preprocess_conversation_stream(conversation: list):
    ...
    FPS = 2
    # make conversation
    start_timestamp, end_timestamp = 0, initial_fps_frames / FPS

    phrase, next_start_from = get_phrase_before_timestamp(
        assistant_text_stream, clip_pts[initial_fps_frames - 1]
    )
    conversation = [
        {
            "role": "user",
            "content": [
                ...
                {"type": "video", "video": clip[:initial_fps_frames]},
                user_query_dict,
            ],
        },
        {
    ...
    frames_list = [clip[:initial_fps_frames]]
    for i in range(initial_fps_frames, len(clip), streaming_fps_frames):
        start_timestamp, end_timestamp = i / FPS, (i + streaming_fps_frames) / FPS
        phrase, next_start_from = get_phrase_before_timestamp(
            assistant_text_stream,
            clip_pts[i + streaming_fps_frames - 1],
            start_from=next_start_from,
        )
        frames = clip[i : i + streaming_fps_frames]
        conversation.extend(
            [
                {
                    "role": "user",
...
class LazySupervisedDataset(Dataset):
    ...
    def process_video(self, video_file):
        """
        1. 优先用 torchcodec 解码（容错好、支持 GPU/NVDEC）。
        2. 若 torchcodec 不可用或解码失败，则回退到 decord。
        返回:
        ...
        if TORCHCODEC_AVAILABLE:
            try:
                decoder = VideoDecoder(video_file, num_ffmpeg_threads=4)
                total_frames = decoder.metadata.num_frames
                avg_fps = decoder.metadata.average_fps
                assert total_frames > 0, "empty video"

                def get_batch(idxs):
                    return decoder.get_frames_at(indices=idxs).data  # (T,H,W,C)

            except Exception as e:
                print("torchcodec failed -> fallback to decord:", e)
                return self._process_video_decord(video_file)
        else:
            return self._process_video_decord(video_file)
        ...
        target_frames = np.clip(
            round(video_length / interval),
            getattr(self.data_args, "video_min_frames", 4),
            getattr(self.data_args, "video_max_frames", 8),
        )
        frame_idx = np.unique(
            np.linspace(0, total_frames - 1, target_frames, dtype=int)
        )
        video_np = get_batch(frame_idx)  # (T,H,W,C)
        fps = len(frame_idx) / video_length

        proc = copy.deepcopy(self.data_args.image_processor)
        proc.max_pixels = self.data_args.video_max_frame_pixels
        proc.min_pixels = self.data_args.video_min_frame_pixels
        proc.size["longest_edge"] = proc.max_pixels
        proc.size["shortest_edge"] = proc.min_pixels

        video_processed = proc.preprocess(
            images=None, videos=video_np, return_tensors="pt"
        )
        video_tensor = video_processed["pixel_values_videos"]  # (T,C,H,W)
        grid_thw = video_processed["video_grid_thw"][0]
        second_per_grid_ts = [
            self.data_args.image_processor.temporal_patch_size / fps
        ] * len(grid_thw)
    ...
    # ----------------------------------------------------------------------
    def _process_video_decord(self, video_file):
        vr = VideoReader(video_file, num_threads=4)
        total_frames = len(vr)
        avg_fps = vr.get_avg_fps()
        ...
        video_np = vr.get_batch(frame_idx).asnumpy()  # (T,H,W,C)
        fps = len(frame_idx) / video_length
